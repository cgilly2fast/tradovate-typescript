import TradovateSocket, { TradovateSocketSubscribeParams } from "./TradovateSocket";
import {Quote, DOM, Histogram, TickPacket, BarPacket, ChartRequest} from "../utils/types"
...
export default class MarketDataSocket extends TradovateSocket {
    public subscriptions: Array<{symbol:string |number, subscription:() => Promise<void>}>
    ...
    async subscribeQuote(params:MarketDataSocketSubscribeParams ) {
        ...
        const isQuote = (data: any) => data.e && data.e === 'md' && data.d && data.d.quotes

        // if contract id is undefined then look up?

        const sendCallback = (id: number, item:any) => {
            
            if(!isQuote(item)) return

            const quotes: Quote[] = item.d.quotes
            
           let filterRes = quotes.filter((quote:Quote) => {return quote.contractId === contractId}).forEach(callback!)
    ...
    async subscribeDOM(params:MarketDataSocketSubscribeParams ) {
        const {symbol, contractId, callback} = params
        const isDom = (data: any) => data.e && data.e === 'md' && data.d && data.d.doms
        ...
        const subscription = await this.request({
            ...
            onResponse: (id, item) => {            
                ...
                doms.filter(dom => {return dom.contractId === contractId}).forEach(callback!)         
            },
            disposer: () => {
                this.request({
    ...
    async subscribeHistogram(params:MarketDataSocketSubscribeParams ) {
        ...
        const isHistogram = (data: any) => data.e && data.e === 'md' && data.d && data.d.histograms
        ...
        const subscription = await this.request({
            ...
            onResponse: (id, item) => {            
                ...
                histograms.filter(histogram => {return histogram.contractId === contractId})
                    .forEach(callback!)          
            },
            disposer: () => {
                this.request({
    ...
    async getChart(params: MarketDataSocketGetChartParams) {
        ...
        const subscription = await this.request({
            ...
            onResponse: (id, item) => {
                ...
                charts.filter(chart => {return chart.id === realtimeId || chart.id === historicalId})
                    .forEach(callback!)            
            },
            disposer: () => {
                this.request({
