...
import { Action, Payload, Item, Dictionary } from '../utils/types'
import Dispatcher from '../utils/dispatcher'
const { REPLAY_URL } = URLs

export const nextReplayPeriod = (state: Dictionary, action: Action): Action => {
    ...
    if (event === 'replay/nextReplayPeriod') {
        ...
        const modified_period = current_period - 1
        if (modified_period < replayPeriods.length) {
            const sessionResults = getSessionResults()
            //set results of this session
            sessionResults.push({
                start: replayPeriods[modified_period].star.start,
                stop: replayPeriods[modified_period].stop,
...
async function startNextReplayPeriod(
    ...
    const originalSocket = replaySocket.getSocket()
    try {
